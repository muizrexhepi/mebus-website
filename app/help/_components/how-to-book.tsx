...
const HowToBook = () => {
  ...
  const bookingProcessSteps = t("pageMetadata.bookingProcess.steps", {
    returnObjects: true,
  }) as string[];
  const bookingTips = t("pageMetadata.bookingTips.tips", {
    returnObjects: true,
  }) as string[];
        ...
        <div className="flex flex-col gap-4 sm:flex-row items-start sm:items-center  justify-between">
          ...
          <Link href="/help">
            <Button variant="outline" className="mb-4">
              ...
              {t("pageMetadata.backButton.label")}
            </Button>
          </Link>
        </div>
        <Card className="mb-8">
          ...
          <CardContent>
            <ol className="list-decimal list-inside space-y-2">
              {bookingProcessSteps?.map((step: string, index: number) => (
        ...
        <Card className="mb-8">
          ...
          <CardContent>
            <ul className="list-disc list-inside space-y-2">
              {bookingTips?.map((tip: string, index: number) => (
                <li key={index}>{tip}</li>
              ))}
            </ul>
          </CardContent>
